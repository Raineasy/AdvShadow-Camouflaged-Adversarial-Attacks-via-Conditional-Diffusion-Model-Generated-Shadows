...
model_path = 'D:\\tree\\ddim\\classifer_model\\resnet-34-classifier.pkl'  # 修改为分类器 .pkl 文件路径
...
if __name__ == "__main__":
    ...
    image_labels_json = 'D:\\tree\\ddim\\image_labels.json'
    ...
    dataset_labels_json = 'D:\\tree\\ddim\\config.json'
    ...
    train_dataset = CustomDataset(
        image_dir='D:\\tree\\ddim\\1',
        mask_dir='D:\\tree\\ddim\\1_mask',  # 假设mask图像存储在'masks'文件夹中
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, masks, labels) in pbar:
            ...
            for image, mask, true_label in zip(images, masks, labels):
                # 非目标攻击，无需指定特定目标标签
                label_index = label_to_int[true_label]
                print(label_index)
                true_label = torch.tensor([label_index], device=device)

                _, _, shadowed_image = gaussian_diffusion.optimize_shadow_position(
                    classifier, image, mask, true_label, device
                )
                shadowed_images_batch.append(shadowed_image)

            # 将单个阴影图像合并为批次
            shadowed_images_batch = torch.stack(shadowed_images_batch, dim=0)
