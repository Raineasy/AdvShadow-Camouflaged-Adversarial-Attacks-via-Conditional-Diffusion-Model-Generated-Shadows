...
def get_dataset(args, distributed=False):
    ...
    dataset_folder = ImageFolder(root=args.dataset_path)
    print("class_to_idx:", dataset_folder.class_to_idx)
    print("First few elements of dataset_folder.imgs:", dataset_folder.imgs[:5])
    image_paths, labels = zip(*[(img[0], dataset_folder.class_to_idx[img[1]]) for img in dataset_folder.imgs])
...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, image_dir, mask_dir, image_paths, labels, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        mask_path = os.path.join(self.mask_dir, 'mask_' + self.image_paths[idx])

        image = default_loader(img_path)
        mask = default_loader(mask_path)  # 假设掩码是图片格式
        ...
        return image, mask, label, self.image_paths[idx]
